...
def search(field, value):
    ...
    if order == "asc":
        cursor.execute(f"select p.id, p.name, p.bought, p.sold, s.name from pet p, animal s, tag t, tags_pets tp where p.species = s.id and t.name = '{value}' and tp.pet = p.id and tp.tag = t.id order by p.{oby}")
    else:
        cursor.execute(f"select p.id, p.name, p.bought, p.sold, s.name from pet p, animal s, tag t, tags_pets tp where p.species = s.id and t.name = '{value}' and tp.pet = p.id and tp.tag = t.id order by p.{oby} desc")
...
def edit(pid):
    ...
    if request.method == "GET":
        ...
        cursor.execute("UPDATE pet SET sold = ?, description = ? WHERE id = ?", [today, description, pid])
        # TODO Handle sold
        conn.commit()
        conn.close()
